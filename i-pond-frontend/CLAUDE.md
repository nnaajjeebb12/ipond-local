...
- **The owner comes from `app_settings.sync_owner_id` first, `SYNC_OWNER_ID` in `.env` second** (`getSyncOwner` in `@/lib/settings`). `.env` is the seed; the Appliance page can change it at runtime (admin-gated) and the change applies to the next sync with no restart. `syncConfig(pool)` is async for this reason.
- **The main server is not to be changed — ever.** It exposes nothing about owners or ponds to a *token* client, but it does to a *logged-in user*, and the Pi uses exactly that: `@/lib/cloudAccount` performs the main server's own NextAuth credentials flow (`/api/auth/csrf` → `/api/auth/callback/credentials` → `/api/auth/session`) and reads `/api/ponds` with the session. `POST /api/settings/cloud-connect` turns that into: owner id + name saved to `app_settings`, ponds reconciled by `pond_code` (**main server wins** on both; main-only created here; local-only reported). Viewer accounts refused; admin accounts set the owner but skip the import (`/api/ponds` returns every pond for admins). Password used once, never stored or logged. Needs the server reachable. Manual UUID entry (admin-gated, unverified) remains the offline fallback. A wrong owner or a missing pond is not data loss — the worker returns `pond_mismatch`, marks nothing, and names the pond codes involved.
- If the main server's login ever changes (Auth.js upgrade, cookie names, 2FA), `cloudSignIn` is the one place to fix; success is judged by `/api/auth/session` returning a user, not by redirect targets.
- **Owners = users on the main server.** One table, `owners`: `id` (this is `SYNC_OWNER_ID`), `name`, `email`, `password_hash`, `role`, `expires_at`. The appliance's owner is simply the customer's login account there.
- **The real receiver** (`cloned main/…/src/app/api/sync/route.ts`, commit b0c3b75; mirrored verbatim in `src/app/api/sync/route.ts` here) answers **`{ ok, inserted, skipped }`** — nothing else. `inserted` = rows it placed, **duplicates included** (ON CONFLICT drops them but the counter still ticks), so `inserted` means "on the server now". `skipped` = no `ponds` row with that `owner_id` + `pond_code` → **NOT on the server**. It matches on **`ponds.owner_id`**, not `user_pond_access`. It loops row by row (2 queries each) — fine at 500/batch.
- **The worker treats `skipped > 0` as "some rows are not on the server" and never marks blindly.** The receiver does not say *which* rows, so `runSync` probes one reading per pond code in the batch (a re-send is a free no-op), learns which codes the server lacks, marks only the others, and excludes the unknown codes from later batches this run. One pond missing on the main server therefore delays only that pond, not the site. Result carries `unknownPonds`; reason is `pond_mismatch` only when nothing at all could ship (almost always a wrong owner). Prefer an `unknown` field if a receiver ever sends one.
- **Main-server DB prerequisites per site (data, not code — the main server's code is never changed):** (1) every pond the Pi will sync must exist there with **`ponds.owner_id` set to the site's owner UUID** — the admin console's create route leaves `owner_id` NULL, so `UPDATE ponds SET owner_id = '<uuid>' WHERE pond_code = 'PND-0NN'` is required per pond; (2) a **unique index on `sensor_readings (pond_id, time)`** must exist or `ON CONFLICT` makes every batch 500 (`server_error` in the sidebar). Neither is in the main repo's migrations.
...
- `GET /api/settings/owner` — `{ ownerId, source: db|env|none, name, target, admin, online, serverReachable }`. Reachability probe 3 s, cached 30 s.
- `POST /api/settings/cloud-connect` — `{ email, password }` of a main-server account → signs in there, saves owner id + name, imports the account's ponds. Returns `{ owner, ponds: { cloudCount, updated, created, localOnly, uncoded }, note }`. 503 offline/unreachable, 401 invalid credentials, 403 viewer / subscription expired, 502 main-server trouble.
- `PUT /api/settings/owner` — **admin cookie required** (401 `admin_required`). `{ ownerId (UUID), name (required) }` → saved to `app_settings` **without** remote verification (there is nothing on the main server to verify against). 400 `invalid_owner_id` / `name_required`.
...
### Cloud sync
- `POST /api/sync` — **main server only** (verbatim copy of the real one here — do not edit it). Bearer `SYNC_TOKEN`. Body `{ readings: [{ time, owner_id, pond_code, temperature, ph, salinity, dissolved_oxygen }] }`, never empty (400). Per row: `SELECT id FROM ponds WHERE owner_id = $1 AND pond_code = $2`, then insert with `ON CONFLICT (pond_id, time) DO NOTHING`. Returns `{ ok, inserted, skipped }`.
...
- **Sensor**: every reading in the last **30 minutes** out of range, with at least **7** readings in that window → INSERT `sensor_alerts`. (Was "7 consecutive readings" — under a minute at the USB gateway's cadence.) **Auto-resolves** (`resolved_at`) as soon as one in-range reading appears in the window, so the popup and badge let go without a click.
- **Connectivity**: 20+ mins no data → INSERT `sensor_alerts` with `sensor='connectivity'` (auto-resolves on next data). **Ponds that have never sent a reading are not alerted on** — no gateway, nothing to lose; they still show as offline. Both the worker and `/api/ponds/status` apply this; the status route now reads the TRUE last reading per pond (LATERAL `ORDER BY time DESC LIMIT 1`, index-only) instead of "last reading in the past 25 minutes", which had made an hour-offline pond indistinguishable from a never-seen one.
- **Re-alert cooldown, `REALERT_COOLDOWN = 24 hours`**, in the worker (`mayRaise`) and `/api/ponds/status`: the same pond + sensor is not raised again while an alert for it is open OR was acknowledged inside the cooldown. Before this, acknowledging a pond that stayed offline brought a fresh alert back on the next 30 s status poll (and every 5 min from the worker) — the popup was effectively un-dismissable. Keep the two constants in step.
- **Worker**: `scripts/alert-worker.ts`, cron `*/5 * * * *`.
- **Popup**: shows unacknowledged, unresolved alerts. Two exits with different meanings — **Acknowledge** (server-side, clears the badge, shows as acknowledged in `/notifications`) and **Ignore** (this browser only: alert stays open on the server, still counted in the badge, still active in `/notifications`). **Either one suppresses the CONDITION — `pondId:sensor`, not the alert id — for the rest of the browser session** (`sessionStorage` key `ipond.alertsHandledThisSession`), so a fresh alert row for the same pond + sensor does not pop again until the tab is closed. A new tab or a new day starts clean. "Ignore all" / "Acknowledge all" in the popup header; per-row and "Acknowledge all" on `/notifications`.
- **Popup is optimistic**: the card disappears on click; the acknowledge request and the view refresh run in the background, and only a failure brings the card back with a message. Before this the button waited for the POST plus three refetches, which on a loaded Pi took seconds.
...
- Never insert a `sensor_alerts` row without the `mayRaise` guard (open OR acknowledged within `REALERT_COOLDOWN`) — there is no unique index to catch duplicates, and a bare `acknowledged_at IS NULL` check re-raises the moment someone acknowledges.
- Never suppress popups by alert id — suppress by `pondId:sensor`, or a re-raised row nags again.
